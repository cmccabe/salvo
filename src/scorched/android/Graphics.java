...
public class Graphics {
    ...
    public void scrollBy(float x, float y) {
        ...
        if (newRight > MaxX) newX = MaxX - (mCanvasWidth / mV.mZoom);
        float MaxY = Model.MAX_ELEVATION + 2*(Model.PLAYER_SIZE + Model.TURRET_LENGTH);
        if (newTop > MaxY) newY = MaxY - (mCanvasHeight / mV.mZoom);

        // We never want to scroll the game below the bottom edge, so after doing the
        // above check, we do the below check
        if (newX < 0f) newX = 0f;
        if (newY < 0f) newY = 0f;
    ...
    public void setSurfaceSize(int width, int height) {
        ...
        // TODO Properly center, reposition canvas to be in bounds, possibly zoom in
